...
class CuentasCog(commands.Cog):
    ...
    async def registrar_cuenta(self, ctx, pais: Option(str, "Elige un país", autocomplete=basic_autocomplete(autocomplete_paises)), moneda: Option(str, "Elige una moneda", autocomplete=basic_autocomplete(autocomplete_moneda)), usuario: Option(str, "Elige un usuario", autocomplete=basic_autocomplete(autocomplete_usuarios)), nombre_cuenta: str, valor_inicial: float):
